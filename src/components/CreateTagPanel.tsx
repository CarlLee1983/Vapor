...
export function CreateTagPanel({ repository, onClose, onCreated }: Props) {
  ...
  const pushPreview = push && plan && hasRemotes ? `git push ${remote} ${plan.tag}` : null;
  ...
  if (isLoading) {
    return <p role="status">Loading tags and configuration...</p>;
  }
  ...
  return (
    <>
      ...
      <footer className="dialog-actions">
        ...
        <button
          type="button"
          disabled={!plan || !preview || isCreating || (push && !hasRemotes)}
